...
export default function AddBoq({
  siteInfo,
  onBoqAdded,
}: {
  siteInfo: any;
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const payload: any = {
        data: {
          bus_station: siteInfo?.documentId
            ? { connect: [siteInfo.documentId] }
            : null,
          division: siteInfo?.division?.documentId
            ? { connect: [siteInfo.division.documentId] }
            : null,
          depot: siteInfo?.depot?.documentId
            ? { connect: [siteInfo.depot.documentId] }
            : null,
          bus_stand: siteInfo?.bus_stands?.length
            ? { connect: [siteInfo.bus_stands[0].documentId] }
            : null,
