              ...
              <div className="flex flex-col space-y-4 sm:flex-row sm:items-center sm:justify-between sm:space-y-0 mb-4 sm:mb-6 gap-4">
                ...
                <div className="flex flex-col sm:flex-row items-center gap-3">
                  ...
                  <p className="text-sm text-gray-600">
                    Showing {pagination[activeTab].total} of{" "}
                    {pagination[activeTab].overallTotal} records
                  ...
                  >
                    <DialogTrigger asChild>
                      <PillButton
                        variant="accent"
                        onClick={() => {
                          console.log("Opening dialog for adding new item");
                          setEditingItem(null);
                          resetForm();
                        }}
                  ...
                  >
                    ...
                    <DialogContent className="w-[95vw] sm:max-w-md bg-gradient-to-br from-amber-50 via-orange-50 to-yellow-50 border-white/30">
                      <DialogHeader>
                        <DialogTitle className="text-lg sm:text-xl font-semibold text-gray-900">
                          Bulk Import {activeTab.replace("-", " ")}
                        </DialogTitle>
                      </DialogHeader>
                      <div className="py-4 space-y-4">
                        ...
                        <p className="text-xs text-gray-600">
                          Required: name (must be unique). Duplicate names in
                          the Excel file or system will be skipped. Optional
                          fields will be included if provided, or saved as
                          empty/default.
                          {activeTab === "divisions" &&
                            " Optional: code (unique if provided), region (defaults to 'Western'), description."}
                          {activeTab === "depots" &&
                            " Optional: code (unique if provided), division (defaults to 'Ahilyanagar' or first division if not provided), address, contactPerson, phone."}
                          {activeTab === "bus-stations" &&
                            " Optional: division (name or ID), depot (name or ID), address, latitude, longitude, facilities (comma-separated)."}
                          {activeTab === "bus-stands" &&
                            " Optional: platformNumber, division (name or ID), depot (name or ID), busStation (name or ID), capacity, type."}
                        </p>
                        <div className="flex justify-end space-x-3 pt-4 border-t border-white/30">
                          <PillButton
                            variant="secondary"
