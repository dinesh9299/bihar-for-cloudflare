...
import BoqModal from "@/boqcomponents/techboqmodal";
...
type Boq = {
  ...
  users_permissions_user?: { id: number; username: string; email: string }; // ✅ relation
};
...
export default function App() {
  ...
  useEffect(() => {
    ...
    api
      ...
      .then((res) => {
        ...
        setRole(res.data.role?.name || ""); // 👈 save role
      })
      .catch((err) => console.error("Failed to fetch user:", err));
  }, []);

  useEffect(() => {
    if (!createdby) return; // wait until username is ready

    if (role === "superadmin" || role === "purchase") {
      fetchAllBoqs(); // superadmin sees everything
    } else {
      fetchBoqs(1); // admin/technician sees only their BOQs
    }
  }, [createdby, role]);
    ...
    try {
      ...
      const res = await api.get(
        `/boqs?pagination[page]=${newPage}&pagination[pageSize]=${newPageSize}` +
          `&filters[createdByName][$eq]=${createdby}` + // 👈 only this user
  ...
  const handleSearch = async (value: string) => {
    ...
    try {
      ...
      let url = `/boqs?pagination[pageSize]=1000&populate=*`;

      if (role === "superadmin") {
        url += `&filters[bus_station][name][$containsi]=${value}`;
      } else {
        url +=
          `&filters[$and][0][createdByName][$eq]=${createdby}` +
          `&filters[$and][1][bus_station][name][$containsi]=${value}` +
          `&populate[division]=true&populate[depot]=true&populate[bus_station]=true&populate[bus_stand]=true` +
          `&populate[nvr_selection][populate][nvr]=true&populate[camera_selection][populate][camera]=true` +
          `&populate[switch_selection][populate][switch]=true&populate[rack_selection][populate][rack]=true` +
          `&populate[pole_selection][populate][pole]=true&populate[wpf_selection][populate][weatherproof_box]=true` +
    ...
    try {
      ...
      for (let i = 0; i < rows.length; i++) {
        ...
        const finalBoqData = {
          ...boqData,
          approved: false,
          createdByName: createdby || "Unknown",
        };

        try {
          await api.post(`/boqs`, { data: finalBoqData });
            ...
            <div className="max-h-[70vh] overflow-y-auto">
              <BoqModal
                ...
                onBoqUpdated={async () => {
                  try {
                    const res = await api.get(
                      `/boqs/${selectedBoq.documentId}?populate[division]=true&populate[depot]=true&populate[bus_station]=true&populate[bus_stand]=true&populate[nvr_selection][populate][nvr]=true&populate[camera_selection][populate][camera]=true&populate[switch_selection][populate][switch]=true&populate[rack_selection][populate][rack]=true&populate[pole_selection][populate][pole]=true&populate[wpf_selection][populate][weatherproof_box]=true&populate[cable_selection][populate][cable]=true&populate[conduit_selection][populate][conduit]=true&populate[wire_selection][populate][wire]=true&populate[ups_selection][populate][up]=true&populate[lcd_selection][populate][lcd]=true`
                    );
                    setSelectedBoq(res.data.data); // 🔄 refresh modal data
                    await fetchBoqs(); // 🔄 also refresh the list
                  } catch (err) {
                    console.error("Failed to reload BOQ:", err);
                  }
                }}
              />
            </div>
