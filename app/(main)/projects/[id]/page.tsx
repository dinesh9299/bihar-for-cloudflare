    ...
    <div className=" space-y-6">
      ...
      >
        ...
        <Panel
          ...
          key="2"
        >
          {!project.users_permissions_users ||
          project.users_permissions_users.length === 0 ? (
            <p className="text-gray-500">No users assigned yet.</p>
          ) : (
            <ul className="divide-y divide-gray-100">
              {project.users_permissions_users.map((user: any) => (
                <li
                  key={user.id}
                ...
                >
                  ...
                  <div className="flex items-center gap-2">
                    {user.role?.name && (
                      <Tag color="geekblue">{user.role.name}</Tag>
                    )}
                    <Popconfirm
                      title="Remove user?"
                      okText="Yes"
