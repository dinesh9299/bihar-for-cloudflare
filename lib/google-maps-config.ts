...
export const createMarkerIcon = (status: string, size = 40) => {
  const color = MARKER_COLORS[status as keyof typeof MARKER_COLORS] || MARKER_COLORS.default

  return `data:image/svg+xml;charset=UTF-8,${encodeURIComponent(`
    <svg width="${size}" height="${size}" viewBox="0 0 ${size} ${size}" fill="none" xmlns="http://www.w3.org/2000/svg">
      <circle cx="${size / 2}" cy="${size / 2}" r="${size / 2 - 2}" fill="${color}" stroke="white" strokeWidth="4"/>
...
export const createInfoWindowContent = (survey: any) => {
  return `
    <div style="max-width: 300px; padding: 12px; font-family: system-ui, -apple-system, sans-serif;">
      <h3 style="margin: 0 0 8px 0; font-weight: 600; color: #1f2937; font-size: 16px;">${survey.location}</h3>
      ...
      <div style="display: flex; gap: 8px; margin-bottom: 8px; flex-wrap: wrap;">
        ...
        <span style="background: ${getStatusBadgeStyle(survey.status).bg}; color: ${getStatusBadgeStyle(survey.status).color}; padding: 4px 8px; border-radius: 12px; font-size: 12px; font-weight: 500;">
      ...
      <div style="margin-top: 8px; padding-top: 8px; border-top: 1px solid #e5e7eb;">
        <p style="margin: 0; color: #9ca3af; font-size: 11px;">Last updated: ${survey.lastUpdated}</p>
      </div>
    </div>
  `
}
